...
from composite_adv.utilities import make_dataloader, EvalModel, load_CIFAR10_C
from math import pi
import warnings
...
#                     help='attack names')
parser.add_argument('corruptions', metavar='corruptions', type=str, nargs='+',
                    help='corruption names')
...
parser.add_argument('--corruption', default='', type=str, help='corruption type for corruption data')
parser.add_argument('--severity', default=-1, type=int, help='severity level for corruption data')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    from composite_adv.utilities import make_model, make_madry_model, make_trades_model
    ...
    corruption_names: List[str] = args.corruptions
    corruptions = []
    for corruption_name in corruption_names:
        # tmp = eval(corruption_name)
        corruptions.append(corruption_name)
    ...
    evaluate_ood(model, corruptions, args)
...
def evaluate_ood(model, corruptions, args):

    num_ex = 10000

    model.eval()
    batches_correct: Dict[str, List[torch.Tensor]] = \
        {corr_name: [] for corr_name in corruptions}
    batches_time_used: Dict[str, List[torch.Tensor]] = \
        {corr_name: [] for corr_name in corruptions}

    for corr_name in corruptions:
        batch_tic = time.perf_counter()
        x_test, y_test = load_CIFAR10_C(num_ex, corr_name, args.severity)
        x_test, y_test = x_test.cuda(), y_test.cuda()

        acc = Accuracy(model, x_test, y_test, args.batch_size)
        err = 1. - acc
        print(f"acc % [{corr_name}{args.severity}]: {acc:.2%}")
        batches_correct[corr_name].append(torch.tensor(acc))
        batch_toc = time.perf_counter()
        time_used = torch.tensor(batch_toc - batch_tic)
        batches_time_used[corr_name].append(time_used)

    print('OVERALL')
    accuracies = []
    # attack_success_rates = []
    total_time_used = []
    corruption_correct: Dict[str, torch.Tensor] = {}
    for corruption_name in corruptions:
        print(batches_correct[corruption_name])
        print(batches_time_used[corruption_name])
        # corruption_correct[corruption_name] = torch.cat(batches_correct[corruption_name])
        accuracy = batches_correct[corruption_name][0].float().mean().item()
        # attack_success_rate = 1.0 - corruption_correct[corruption_name][ori_correct[attack_name]].float().mean().item()
        time_used = batches_time_used[corruption_name][0].item()
        print(f'ATTACK {corruption_name}',
            f'accuracy = {accuracy * 100:.1f}',
            # f'attack_success_rate = {attack_success_rate * 100:.1f}',
            f'time_usage = {time_used:0.2f} s',
            sep='\t')
    ...
    with open(args.output, 'a+') as out_file:
        out_csv = csv.writer(out_file)
        out_csv.writerow([args.message])
        out_csv.writerow(['corruption_type'] + corruptions)
        out_csv.writerow(['accuracies'] + accuracies)
        # out_csv.writerow(['attack_success_rates'] + attack_success_rates)
        out_csv.writerow(['time_usage'] + total_time_used)
        out_csv.writerow(['severity_level', args.severity])
        out_csv.writerow(['batch_size', args.batch_size])
        out_csv.writerow(['num_batches', args.num_batches])
        out_csv.writerow([''])
